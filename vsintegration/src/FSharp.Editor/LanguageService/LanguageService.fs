...
//    override this.CreateEditorFactories() = this.CreateEditorFactories()// seq { yield FSharpEditorFactory(this) :> IVsEditorFactory }
...
//type internal FSharpLanguageService(package : FSharpPackage) =
    ...
    //member __.ContentTypeName = FSharpConstants.FSharpContentTypeName
    //member __.LanguageName = FSharpConstants.FSharpLanguageName
    //member __.RoslynLanguageName = FSharpConstants.FSharpLanguageName

    //member __.LanguageServiceId = new Guid(FSharpConstants.languageServiceGuidString)
    //member __.DebuggerLanguageId = DebuggerEnvironment.GetLanguageID()

    ////member __.CreateContext(_,_,_,_,_) = raise(System.NotImplementedException())
