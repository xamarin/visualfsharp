...
* _Type checking context/state_, see for example [`TcState` in CompileOps.fs](https://github.com/dotnet/fsharp/blob/master/src/fsharp/CompileOps.fsi) and its constituent parts, particularly `TcEnv` in [CheckExprs.fs](https://github.com/dotnet/fsharp/blob/master/src/fsharp/CheckExprs.fsi) and `NameResolutionEnv` in [NameResolution.fs](https://github.com/dotnet/fsharp/blob/master/src/fsharp/NameResolution.fsi). A set of tables representing the available names, assemblies etc. in scope during type checking, plus associated information.
...
* _Sequentially type checking files_, see [CheckDecls.fsi](https://github.com/dotnet/fsharp/blob/master/src/fsharp/CheckDecls.fsi)/[CheckDecls.fs](https://github.com/dotnet/fsharp/blob/master/src/fsharp/CheckDecls.fs). Accepts an AST plus a type checking context/state and produces new Typed Tree nodes
  ...
  checking syntactic types and expressions, see [CheckExprs.fsi](https://github.com/dotnet/fsharp/blob/master/src/fsharp/CheckDecls.fsi)/[CheckExprs.fs](https://github.com/dotnet/fsharp/blob/master/src/fsharp/TypeCCheckDeclshecker.fs) including the state held across the checking of a file (see `TcFileState`) and the
